...
export class DynamoDBAPIService {
  ...
  getDeviceReadingsEndPoint: any =
    environment.DynamoBDEndPoints.API_PATHS.getDeviceReadings;
  token: any;
  constructor(
  ...
  async asyncGenericGetMethods(url: string) {
    const resultt = await this.httpClient
      .get(url)
      .pipe(map((data: any[]) => {}))
      .toPromise();
    return resultt;
  }
  async multipleGenericGetMethods(
    ...
    const calls = [];
    for (const iterator of ConnectionsArray) {
      if (iterator === connectionName) {
        calls.push(this.httpClient.get(DevicesUrl).pipe(retry(3)));
      }
    }
